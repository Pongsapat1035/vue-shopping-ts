...
onMounted(async () => {
    try {
        ...
        if (response) {
            const { coverImg, name, quantity, price, detail, colors, sizes } = response;
        ...
        const checkEnableColor = productData.colors.some(color => color.isCheck === true)
        checkColorState.value = checkEnableColor ? true : false
        // console.log('check enable color from array :', checkColorState.value)
        const checkEnableSize = productData.sizes.some(size => size.isCheck === true)
    ...
    <SellerLayout>
        <form class="w-3/4 p-5" @submit.prevent="handleSubmit">
            ...
            <div class="flex flex-col gap-5">
                ...
                <div class="flex gap-5">
                    <div class="flex-auto">
                        <InputTag title="Quality" name="quantity" type="number" placeHolderText="quantity"
                            v-model:value="productData.quantity" v-model:error="inputError.quantity"></InputTag>
                    </div>
                    <div class="flex-auto">
                        <InputTag title="Price" name="price" type="number" placeHolderText="price"
                            v-model:value="productData.price" v-model:error="inputError.price"></InputTag>
                    </div>
                </div>
                ...
                <div class="flex gap-5">
                    <ColorSelectField v-model:colors="productData.colors" v-model:enable="checkColorState">
                    </ColorSelectField>
                    <SizeSelectField v-model:sizes="productData.sizes" v-model:enable="checkSizeState">
                    </SizeSelectField>
                </div>
            </div>
        </form>
